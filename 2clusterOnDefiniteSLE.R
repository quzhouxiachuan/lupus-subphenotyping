...
#delete duplicated patients;  and # a complete script refer to medinfo code secured folder. 
...
enc[1,1] = '' #a complete script refer to medinfo code folder
...
colnames(test) = c('SEX', 'RACE', 'RASH', 'PHOTO', 'JOINT', 'SERO', 
                   'RENAL', 'NEURO','HEME',
                   'ANA', 'IMMUN','ULCER', 'DXAGE1', 'index')
dat3 <- melt(test, id.var = 'index')
ggplot(dat3, aes(variable, index)) + geom_tile(aes(fill = factor(value)),
colour = "white") + scale_fill_manual(values=c("yellow", "red", "black"))
