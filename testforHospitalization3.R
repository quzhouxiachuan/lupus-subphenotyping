...
#delete subjects with NULL values 

#mydata = mydata[mydata$DXDATE !='',]
x = na.omit(x)
x = x[complete.cases(x), ]
x = x[x$MRN !='08334448',] #08334448 has NA in the RACE column 
...
lc_test = lc3 
res = cbind(lc_test$probs$rash[4:6],lc_test$probs$ACRPHOTO[4:6],lc_test$probs$ACRJOINT[4:6], lc_test$probs$ACRSERO[4:6]
            ,lc_test$probs$ACRRENAL[4:6],lc_test$probs$ACRNEURO[4:6],lc_test$probs$ACRHEME[4:6]
            ,lc_test$probs$ACRANA[4:6],lc_test$probs$ACRIMMUN[4:6],lc_test$probs$ACRULCER[4:6],
             lc_test$probs$DXAGE1[4:6], lc_test$probs$SEX[4:6]
            ,lc_test$probs$RACE[4:6], lc_test$P)
colnames(res) = c('rash', 'ACRPHOTO','ACRJOINT','ACRSERO','ACRRENAL','ACRNEURO'
                  ,'ACRHEME','ACRANA','ACRIMMUN','ACRULCER','DXAGE1', 'SEX', 'RACE'
                  ,'population_share')
...
colnames(kk)[1] = 'class'
with(kk,table(class,'ACRPHOTO'))

with(kk, chisq.test(ACRPHOTO, class))
with(kk, chisq.test(SEX, class))
with(kk, chisq.test(RACE, class))
with(kk, chisq.test(ACRJOINT, class))
with(kk, chisq.test(ACRSERO, class))
with(kk, chisq.test(ACRRENAL, class))
with(kk, chisq.test(ACRNEURO, class))
with(kk, chisq.test(ACRHEME, class))
with(kk, chisq.test(ACRANA, class))
with(kk, chisq.test(ACRIMMUN, class))
with(kk, chisq.test(ACRULCER, class))
with(kk, chisq.test(DXAGE1, class))
with(kk, chisq.test(rash, class))
